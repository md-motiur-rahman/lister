...
if (!databaseUrl) {
  throw new Error("DATABASE_URL is required. Example: DATABASE_URL=... npm run seed:list");
}
...
async function seed() {
  ...
  for (const item of seedItems) {
    await sql`
      INSERT INTO list_item (id, name, category, quantity, purchased, priority, updated_at)
      VALUES (
        ${crypto.randomUUID()},
        ${item.name},
        ${item.category},
